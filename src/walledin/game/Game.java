...
public class Game implements RenderListener {
    ...
    public void update(int ndt) {
        ...
        for (int i = 0; i < mWalls.size(); i++) {
            if (mPlayer.getBoundRect().addOffset(vNewPos).intersects(mWalls.get(i))) {
                vNewPos = mPlayer.getPos(); // do no update
            }
        }
        /* Update player position */
        if (java.lang.Math.abs(vNewPos.x - mPlayer.getPos().x) > 0.1f)
            mPlayer.Move(vNewPos.x - mPlayer.getPos().x); // FIXME, used only for foot animation
        mPlayer.setPos(vNewPos);

        if (Input.getInstance().keyDown(KeyEvent.VK_SPACE)) {
            mWalls.add(new Rectangle(mPlayer.getPos().x() + 65, mPlayer.getPos().y(), 30, 90));
    ...
    public void draw(Renderer renderer) {
        renderer.drawRect("sun", new Rectangle(60, 60, 64, 64)); // draw background
        ...
        for (int i = 0; i < mWalls.size(); i++) {
            renderer.drawRect("wall", new Rectangle(0.0f, 0.0f, 110 / 128.0f, 235 / 256.0f), mWalls.get(i));
        }
